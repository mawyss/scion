...
func TestHVFVerification(t *testing.T) {
	...
	assert.True(t, libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp, false))
	assert.False(t, libepic.VerifyHVF(randBytes(16), epicpath, s, timestamp, false))
	assert.False(t, libepic.VerifyHVF(authPenultimate, nil, s, timestamp, false))
	assert.False(t, libepic.VerifyHVF(authPenultimate, epicpath, nil, timestamp, false))
	assert.False(t, libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp-10, false))
	assert.False(t, libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp, true))
	...
	assert.True(t, libepic.VerifyHVF(authLast, epicpath, s, timestamp, true))
	assert.False(t, libepic.VerifyHVF(randBytes(16), epicpath, s, timestamp, true))
	assert.False(t, libepic.VerifyHVF(authLast, nil, s, timestamp, true))
	assert.False(t, libepic.VerifyHVF(authLast, epicpath, nil, timestamp, true))
	assert.False(t, libepic.VerifyHVF(authLast, epicpath, s, timestamp+10, true))
	assert.False(t, libepic.VerifyHVF(authLast, epicpath, s, timestamp, false))
}
